...
Detects your OS+arch, pulls the latest release from GitHub, drops a ~3 MB binary into `~/.local/bin`. Customize with `RESMAN_INSTALL_DIR=/usr/local/bin` or `RESMAN_VERSION=v0.17.11`. Each release also ships a `.sha256` the installer verifies automatically; a failed or missing verification aborts the install (override with `RESMAN_ALLOW_UNVERIFIED=1`).
...
The project is at **v0.17.11**. Through v0.9–v0.17 the following shipped:
